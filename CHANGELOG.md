...
### Fixed
- **Upgrades could silently target a different global install than the version check/launcher on machines with multiple npm prefixes.** When both a system Node global dir (`%APPDATA%\npm`) and an `fnm`-managed global dir existed (each with its own global `node_modules`), `npm install -g` via the system npm updated one prefix while version detection and service launch resolved the command by PATH order and could hit the *other*, stale prefix — so the card kept showing the old version after a successful upgrade (observed with codex 0.153.4 vs 0.154.0). Command resolution now prefers the global bin of the npm that actually upgrades each service (`which_service` / `resolve_service` / `npm_global_bin`), with PATH/fnm kept as fallback; git-based hermes is unaffected.
- Post-upgrade version check retries with capped linear backoff instead of reading once. This covers the secondary case where npm exits 0 while a large native binary (e.g. codex's ~300 MB `codex.exe`) is still being moved/scanned by Defender, so an immediate `--version` can briefly read the stale binary. Stage 4 now waits until the version advances (npm packages) or becomes readable (git-based hermes) and warns clearly instead of silently showing the old version.
